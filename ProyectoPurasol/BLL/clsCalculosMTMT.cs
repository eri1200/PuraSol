...
namespace BLL
{
    public class clsCalculosMTMT
    {
        ...
        public void calculosMTMT(double PPREne, double PPRFeb, double PPRMar, double PPRAbr, double PPRMay, double PPRJun, double PPRJul, double PPRAgo, double PPRSep, double PPROct, double PPRNov, double PPRDic, 
            ...
            double PromedioRecibos, double PromedioSGIS, double crecimientoAnual, string Compania, string Tarifa, clsCurvasMTMT curvas, List<double> pico)
        {
            ...
            for (int i = 0; i < 26; i++)
            {
                ...
                for (int j = 1; j < 13; j++)
                {
                    if (i == 0)
                    {
                        enerCoberturaRealP[i, j] = curvas.cResumen[j-1, 11];
                    }
                    else
                    {
            ...
            for (int i = 0; i < 26; i++)
            {
                ...
                for (int j = 1; j < 13; j++)
                {
                    if (i == 0)
                    {
                        compraRegP[i, j] = curvas.cResumen[j-1, 13] * calculos[i,6] / calculos[i,3];
                    }
                    else
                    {
                        compraRegP[i, j] = compraRegP[0, j] * calculos[i, 6] / calculos[i, 3];
                    }
                }

                compraRegP[i, 13] = compraRegP[i, 1] + compraRegP[i, 2] + compraRegP[i, 3] + compraRegP[i, 4] + compraRegP[i, 5] + compraRegP[i, 6] + compraRegP[i, 7] + compraRegP[i, 8] +
                        compraRegP[i, 9] + compraRegP[i, 10] + compraRegP[i, 11] + compraRegP[i, 12];
            ...
            for (int i = 0; i < 26; i++)
            {
                ...
                for (int j = 1; j < 13; j++)
                {
                    if (i == 0)
                    {
                        enerCoberturaRealV[i, j] = curvas.cResumen[j - 1, 18];
                    }
                    else
                    {
            ...
            for (int i = 0; i < 26; i++)
            {
                ...
                for (int j = 1; j < 13; j++)
                {
                    if (i == 0)
                    {
                        compraRegV[i, j] = curvas.cResumen[j - 1, 20] * calculos[i, 6] / calculos[i, 3];
                    }
                    else
                    {
                        compraRegV[i, j] = compraRegV[0, j] * calculos[i, 6] / calculos[i, 3];
                    }
                }

                compraRegV[i, 13] = compraRegV[i, 1] + compraRegV[i, 2] + compraRegV[i, 3] + compraRegV[i, 4] + compraRegV[i, 5] + compraRegV[i, 6] + compraRegV[i, 7] + compraRegV[i, 8] +
                        compraRegV[i, 9] + compraRegV[i, 10] + compraRegV[i, 11] + compraRegV[i, 12];
            ...
            for (int i = 0; i < 26; i++)
            {
                ...
                for (int j = 1; j < 13; j++)
                {
                    if (i == 0)
                    {
                        enerCoberturaRealN[i, j] = curvas.cResumen[j - 1, 24]*calculos[i,3];
                    }
                    else
                    {
            ...
            for (int i = 0; i < 26; i++)
            {
                ...
                for (int j = 1; j < 13; j++)
                {
                    if (i == 0)
                    {
                        compraRegN[i, j] = curvas.cResumen[j - 1, 26];
                        compraRegN[i, 13] =consumoEnergiaN[12]- enerCoberturaRealN[2, 12];
                    }
                    else
                    {
                        compraRegN[i, j] = compraRegN[0, j] * calculos[i, 6];
                        compraRegN[i, 13] = compraRegN[0, 13] * calculos[i, 6];
                    }
                }
            ...
            for (int i = 0; i < 26; i++)
            {
                ...
                for (int j = 1; j < 13; j++)
                {
                    if (i==0)
                    {
                        enerCoberturTarAcc[i, j] = curvas.cResumen[j-1,4] *calculos[i,3];
                    }
                    else
                    {
            ...
            for (int i = 0; i < 26; i++)
            {
                ...
                for (int j = 1; j < 13; j++)
                {
                    ...
                    {
                        if ((sobranteTA[i, j-1] + enerProducida[i, j])> sobranteTA[i, j - 1] + enerProducida[i, j] - (consumoEnergiaP[j - 1] + consumoEnergiaV[j - 1] + consumoEnergiaN[j - 1]) * calculos[i, 2])
                        {
            ...
            //llenar la matriz demanda potencia
            ...
            for (int j = 0; j < 12; j++)
            {
                demandaPotP.Add(curvas.cResumen[j, 14]);
                demandaPotV.Add(curvas.cResumen[j, 21]);
                demandaPotN.Add(curvas.cResumen[j, 27]);
            }
            ...
            for (int i = 0; i < 12; i++)
            {
                maxP = Math.Max(maxP, demandaPotP[i]);
                maxV = Math.Max(maxV, demandaPotV[i]);
                maxN = Math.Max(maxN, demandaPotN[i]);
            }

            demandaPotP.Add(maxP);
            demandaPotV.Add(maxV);
            demandaPotN.Add(maxN);
            ...
            for (int i = 0; i < 26; i++)
            {
                ...
                for (int j = 1; j < 13; j++)
                {
                    if (i == 0)
                    {
                        costoPredichoPot[i, j] = demandaPotP[j - 1] * potMaxBloque[0] + demandaPotV[j - 1] * potMaxBloque[1] + demandaPotN[j - 1] * potMaxBloque[2];
                    }
                    else
                    {
                        costoPredichoPot[i, j] = costoPredichoPot[0, j] * calculos[i, 6];
                    }
